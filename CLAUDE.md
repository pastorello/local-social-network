...
Milestone state: **M0 (recovery & audit), M1 (auth & roles), M2 (map & issue reports) and M3 (polls) are done**; **M4 (showcase) is nearly done** — Gaeta Partecipa rebrand, full Italian copy sweep, auth hardening, deploy prep (docs/deploy.md), README/AI-log-index/LinkedIn docs. See `docs/ai-log/` for what happened in each. Still open in M4: the hosting decision + live demo URL (spec §6) and publishing the LinkedIn draft. The old `posts` and `notifications` apps were **removed in M2** (their patterns live on in `reports`); the tutorial `Question`/`Choice` polls were **replaced in M3** by the spec §7 `Poll`/`PollOption`/`Vote` schema.
